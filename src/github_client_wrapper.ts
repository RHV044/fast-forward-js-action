...
export class GitHubClientWrapper implements GitHubClient {
  ...
  get_current_pull_request_number(): number {
    if (
      ...
      if (
        ...
        throw new Error(
          "Issue is not a pull request! No pull request found in context"
        );
    } else return this.context.payload.issue.number;
  }

  async comment_on_pull_request_async(
    pr_number: number,
    comment: string
  ): Promise<void> {
  ...
  async set_pull_request_status(
    pr_number: number,
    new_status: "error" | "failure" | "pending" | "success"
  ): Promise<void> {
    const pullRequestData = await this.get_pull_request(pr_number);
